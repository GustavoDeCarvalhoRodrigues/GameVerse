...
<section class="hero" style="display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 120px; background: linear-gradient(120deg, #1a2230 60%, #2b8ef9 100%); border-radius: 18px; box-shadow: 0 4px 32px 0 rgba(43,142,249,0.10); margin-bottom: 36px; padding: 24px 16px 18px 16px; text-align: center;">
  <h1 style="font-size: 2rem; font-weight: 800; color: #fff; margin-bottom: 6px; letter-spacing: 0.03em; text-shadow: 0 2px 12px #2b8ef9a0;">Administração de Usuários</h1>
  <p style="font-size: 1.05rem; color: #e7e9ee; max-width: 600px; margin: 0 auto 10px auto;">Gerencie permissões, status e informações dos usuários cadastrados.</p>
</section>

<?php if($m = flash('ok')) echo "<p style='color:#7cc9ff;font-weight:600;text-align:center'>{$m}</p>"; ?>
<div style="overflow-x:auto;">
<input type="text" id="search-usuarios" placeholder="Pesquisar usuários..." style="width:100%;margin-bottom:10px;padding:8px 10px;border-radius:8px;border:1px solid #2b8ef9;background:#222;color:#7cc9ff;">
<table id="tabela-usuarios" style="margin-bottom:18px;min-width:340px;">
  <tr style="background:#181c25;color:#7cc9ff;font-size:1.08em;">
    <th>ID</th><th>Nome</th><th>Email</th><th>Status</th><th>Role</th><th>Ações</th>
  </tr>
    ...
    <tr>
      ...
      <td>
        <form method="post" style="display:flex;gap:6px;align-items:center">
          ...
          <label>
            <input type="checkbox" name="status" <?= $u['status'] ? 'checked':'' ?>> Ativo
          </label>
          <select name="role">
