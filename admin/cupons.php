...
<section class="hero" style="display: flex; flex-direction: column; align-items: center; justify-content: center; min-height: 120px; background: linear-gradient(120deg, #1a2230 60%, #2b8ef9 100%); border-radius: 18px; box-shadow: 0 4px 32px 0 rgba(43,142,249,0.10); margin-bottom: 36px; padding: 24px 16px 18px 16px; text-align: center;">
  <h1 style="font-size: 2rem; font-weight: 800; color: #fff; margin-bottom: 6px; letter-spacing: 0.03em; text-shadow: 0 2px 12px #2b8ef9a0;">Administração de Cupons</h1>
  <p style="font-size: 1.05rem; color: #e7e9ee; max-width: 600px; margin: 0 auto 10px auto;">Gerencie cupons de desconto, crie, edite ou remova cupons promocionais.</p>
</section>
...
<div class="admin-grid">
  <div>
    <form method="post" style="background:#181c25;padding:18px 16px;border-radius:14px;box-shadow:0 2px 12px 0 rgba(20,30,50,0.10);">
      ...
      <label style="font-weight:600;color:#7cc9ff;">Valor</label><input name="valor" type="number" step="0.01" value="<?= h($edit['valor']) ?>">
      <label style="font-weight:600;color:#7cc9ff;"><input type="checkbox" name="ativo" <?= $edit['ativo'] ? 'checked':'' ?>> Ativo</label>
      <button class="btn" style="margin-top:10px;min-width:120px;"><?= $edit['idcupom'] ? 'Salvar':'Criar' ?></button>
    </form>
  </div>
  <div>
    <input type="text" id="search-cupons" placeholder="Pesquisar cupons..." style="width:100%;margin-bottom:10px;padding:8px 10px;border-radius:8px;border:1px solid #2b8ef9;background:#222;color:#7cc9ff;">
    <table id="tabela-cupons">
      <tr style="background:#181c25;color:#7cc9ff;font-size:1.08em;">
        <th>ID</th><th>Nome</th><th>Tipo</th><th>Valor</th><th>Ativo</th><th>Ações</th>
      </tr>
