...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  header("Location: ../admin/notificacoes.php");
  exit;
}
...
<section class="hero"
  ...
  <h1
    ...
    Administração de Notificações</h1>
  <p style="font-size: 1.05rem; color: #e7e9ee; max-width: 600px; margin: 0 auto 10px auto;">Crie, edite ou remova
    notificações para os usuários do Gameverse.</p>
</section>
...
<div class="admin-grid">
  <div>
    <form method="post"
      ...
      <label style="font-weight:600;color:#7cc9ff;">Título</label><input name="nome" value="<?= h($edit['nome']) ?>"
        required>
      ...
      <div style="display:flex;gap:10px;margin-top:10px;">
        <button class="btn" style="min-width:120px;"><?= $edit['idnotificacoes'] ? 'Salvar' : 'Criar' ?></button>
        <a href="index.php" class="btn alt" style="min-width:120px;text-align:center;display:inline-block;">Voltar</a>
      </div>
    </form>
  </div>
  <div>
    <input type="text" id="search-notificacoes" placeholder="Pesquisar notificações..."
